...
//    }
     ...
     * @return
     */
    public static com.xw.scannerlib.base.Result decodeIdCard2(Bitmap bmp) throws Exception {
        if (bmp == null) return null;
        if (bmp.getWidth() % 2 == 1 || bmp.getHeight() % 2 == 1) {
            bmp = Bitmap.createBitmap(bmp, 0, 0, bmp.getWidth() / 2 * 2, bmp.getHeight() / 2 * 2);
        }
        ...
        try {
            ...
            if (TextUtils.isEmpty(s)) throw new Exception("failure");
            IdCard2Utils.close();
            com.xw.scannerlib.base.Result result = new com.xw.scannerlib.base.Result();
            result.type = s.contains("cardNumber") ? com.xw.scannerlib.base.Result.TYPE_ID_CARD_FRONT : com.xw.scannerlib.base.Result.TYPE_ID_CARD_BACK;
            result.data = s;
            return result;
        } catch (Exception e) {}
        try {
